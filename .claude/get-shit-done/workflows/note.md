...
- **Global scope**: `D:/repos/claude-usage-monitor/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/repos/claude-usage-monitor/.claude/notes/`)
...
2. Glob `D:/repos/claude-usage-monitor/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/repos/claude-usage-monitor/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/repos/claude-usage-monitor/.claude/notes/` — works in any directory
